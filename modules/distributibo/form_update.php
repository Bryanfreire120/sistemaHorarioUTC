...
carrera.Id_carrera AS cod_carrera,
...
if ($result = $conexion->query($sql)) {
	...
	while ($row = mysqli_fetch_array($result)) {
		...
		$_SESSION['CodigoC'][$i] = $row['cod_carrera'];
		$_SESSION['Ciclo1'][$i] = $row['ciclo_nombre'] ;
...
<label>Facultad / Extensión:</label>
	...
	<select name="carrera">
			...
			while ($row = $result->fetch_assoc()) {
				$selected = ($row['Nom_carrera'] == $_SESSION['Carrera1'][0]) ? 'selected' : '';
				echo '<option value="' . $row['Id_carrera'] . '" ' . $selected . '>' . $row['Nom_carrera'] . '</option>';
			}
			?>
		</select><br>
	</div>
